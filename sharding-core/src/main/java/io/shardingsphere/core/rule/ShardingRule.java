...
public class ShardingRule {
    ...
    /**获取分片规则中所有用到的数据库，若存在读写分离就是MasterSlaveDataSource的beanName*/
    ...
    /**默认的分库规则 default NoneShardingStrategy*/
    private final ShardingStrategy defaultDatabaseShardingStrategy;
    /**默认的分表规则 default NoneShardingStrategy*/
    private final ShardingStrategy defaultTableShardingStrategy;
    /**默认的主键生成器 default DefaultKeyGenerator*/
    ...
    /**
     * 创建分库分表规则
     * @param shardingRuleConfig  分片规则配置类
     * @param dataSourceNames 分片分库所涉及的所有DataSource的beanName
     */
    public ShardingRule(final ShardingRuleConfiguration shardingRuleConfig, final Collection<String> dataSourceNames) {
        ...
        for (TableRuleConfiguration each : shardingRuleConfig.getTableRuleConfigs()) {
            /**存在主从，那么分表中的数据节点使用的数据库就是MasterSlaveDataSource的beanName*/
